...
- HTML responses set `Cache-Control: no-cache` and `Vary: HX-Request`, and `/static/*` assets set `Cache-Control: public, max-age=31536000, immutable`, so browsers pick up new asset paths after an upgrade without needing a manual hard reload and HTMX partials no longer share a browser-cache entry with full pages.
