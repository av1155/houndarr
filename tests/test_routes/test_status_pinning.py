    ...
    async def test_instance_with_error_as_latest_is_surfaced(self, seeded_instances: None) -> None:
        async with get_db() as conn:
            await conn.executemany(
                ...
                """,
                [
                    (10, "searched", "2026-04-20T10:00:00.000Z", None),
                    (11, "error", "2026-04-20T11:00:00.000Z", "connection refused"),
                ],
            )
            await conn.commit()
    ...
    async def test_error_cleared_by_later_non_error_row(self, seeded_instances: None) -> None:
        async with get_db() as conn:
            await conn.executemany(
                ...
                """,
                [
                    (10, "error", "2026-04-20T10:00:00.000Z", "boom"),
                    (11, "searched", "2026-04-20T11:00:00.000Z", None),
                ],
            )
            await conn.commit()
            result = await _active_errors(conn, [1])
    ...
    async def test_returns_most_recent_within_limit(self, seeded_instances: None) -> None:
        async with get_db() as conn:
            await conn.executemany(
                ...
                """,
                [
                    (10, "Movie A", "2026-04-20T10:00:00.000Z"),
                    (11, "Movie B", "2026-04-20T11:00:00.000Z"),
                    (12, "Movie C", "2026-04-20T12:00:00.000Z"),
                ],
            )
            await conn.commit()
