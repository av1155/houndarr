...
- Legacy timezone names such as `US/Eastern` and `GB` resolve in `TZ`, so the Allowed Search Window follows the configured zone instead of falling back to UTC. (#822)
- A `TZ` value the container cannot load is named in the startup log instead of silently becoming UTC. (#822)
