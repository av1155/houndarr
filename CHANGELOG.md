...
- Per-instance `Unreleased` count on the dashboard now reflects the real count of monitored items awaiting release, computed at every snapshot refresh from each adapter's wanted list. Previously every non-Whisparr-v3 instance reported zero. (#N)
