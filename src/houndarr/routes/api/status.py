...
async def get_status(request: Request) -> JSONResponse:
    ...
    for inst in instances:
        ...
        if version == "2":
            ...
            # Authoritative snapshot from the supervisor's 10-minute refresh
            # task (schema v13 columns).  Fall back to the in-memory probe
            # sum only when the columns are still zero (instance just enabled,
            # supervisor hasn't primed the snapshot yet).
            column_monitored = int(inst["monitored_total"])
            column_unreleased = int(inst["unreleased_count"])
            entry["monitored_total"] = (
                column_monitored if column_monitored else missing_count + cutoff_count
            )
            entry["unreleased_count"] = column_unreleased
            refreshed = inst["snapshot_refreshed_at"]
            entry["snapshot_refreshed_at"] = str(refreshed) if refreshed else None
