...
class TestInstanceRowStatePill:
    ...
    def test_enabled_no_error_renders_active_pill(
        self, render_macro: Callable[..., str]
    ) -> None:
        src = (
            _IMPORT
            + "{{ instances.instance_row(instance, active_error_ids) }}"
        )
        result = render_macro(
            src, instance=_instance_stub(enabled=True), active_error_ids=[]
        )
    ...
    def test_enabled_with_error_renders_error_pill(
        self, render_macro: Callable[..., str]
    ) -> None:
        src = (
            _IMPORT
    ...
    def test_disabled_renders_disabled_pill(
        self, render_macro: Callable[..., str]
    ) -> None:
        src = (
            _IMPORT
        ...
        result = render_macro(
            src, instance=_instance_stub(enabled=False), active_error_ids=[]
        )
        assert "Disabled" in result
        assert "text-slate-500" in result
    ...
    def test_active_error_ids_none_treated_as_empty(
        self, render_macro: Callable[..., str]
    ) -> None:
        src = (
            _IMPORT
        ...
        result = render_macro(
            src, instance=_instance_stub(enabled=True), active_error_ids=None
        )
        # No error ids ≡ active.
        assert "Active" in result
        assert ">Error<" not in result
...
class TestInstanceRowDataColumns:
    ...
    def test_each_data_col_present(
        self, render_macro: Callable[..., str], data_col: str
    ) -> None:
        src = (
            _IMPORT
            + "{{ instances.instance_row(instance, active_error_ids) }}"
        ...
        result = render_macro(
            src, instance=_instance_stub(), active_error_ids=[]
        )
...
class TestInstanceRowToggleButton:
    ...
    def test_toggle_button_has_hx_attributes(
        self, render_macro: Callable[..., str]
    ) -> None:
        src = (
            _IMPORT
...
class TestInstanceRowEditAndDeleteButtons:
    ...
    def test_delete_button_attrs_include_name_in_confirm(
        ...
        assert 'hx-swap="outerHTML"' in result
        assert (
            'hx-confirm="Delete \'My Sonarr\'? This cannot be undone."'
            in result
        )
...
class TestFormContextAddMode:
    ...
    def test_add_mode_byte_equal(self, render_macro: Callable[..., str]) -> None:
        src = (
            _IMPORT
            + "{% call(form_id) instances.form_context(is_edit=False) %}"
            "BODY-{{ form_id }}-END{% endcall %}"
        )
        expected = (
    ...
    def test_add_mode_ignores_passed_instance_id(
        self, render_macro: Callable[..., str]
    ) -> None:
...
class TestFormContextCallerArg:
    ...
    def test_caller_receives_form_id_in_edit_mode(
        self, render_macro: Callable[..., str]
    ) -> None:
        src = (
            _IMPORT
            + "{% call(form_id) instances.form_context(is_edit=True, instance_id=7) %}"
            "FID:{{ form_id }}{% endcall %}"
        )
        result = render_macro(src)
        assert "FID:edit-form-7" in result

    def test_caller_receives_form_id_in_add_mode(
        self, render_macro: Callable[..., str]
    ) -> None:
        src = (
            _IMPORT
