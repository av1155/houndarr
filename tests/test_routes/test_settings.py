...
def test_toggle_enabling_clears_stale_error_immediately(app: TestClient) -> None:
    """Re-enabling an errored instance flips Status to Active in the same response.

    Before this fix, the status pill rendered "Error" for 10+ seconds
    after clicking Enable because the newest search_log row was still
    the stale error from before the disable.  The toggle handler now
    writes a system info row on the disabled -> enabled transition so
    active_error_instance_ids(), which keys off newest row per
    instance, sees a non-error row and flips the pill to Active on
    the very HTMX response this click generates.
    """
    import asyncio
    ...
    async def _seed_error(iid: int) -> None:
        async with get_db() as db:
            await db.execute(
                "INSERT INTO search_log (instance_id, action, timestamp, message)"
                " VALUES (?, 'error', ?, 'simulated outage')",
                (iid, datetime.now(UTC).isoformat()),
            )
            await db.commit()

    # Disable the instance (it shipped enabled by default).
    app.post("/settings/instances/1/toggle-enabled", headers=csrf_headers(app))
    # Now simulate the "remote was down, we logged an error" state.
    asyncio.run(_seed_error(1))
    # Re-enable.
    resp = app.post("/settings/instances/1/toggle-enabled", headers=csrf_headers(app))
    assert resp.status_code == 200
    # Status pill renders Active, not Error, in this response.
    assert b"status-pill--active" in resp.content
    assert b"status-pill--error" not in resp.content


def test_toggle_disabling_does_not_write_info_row(app: TestClient) -> None:
    """The info-row write is scoped to the disabled -> enabled transition only.

    Disable click should be a pure config mutation; it must not spam
    search_log with a row per click.
    """
    import asyncio
    ...
    async def _count_info_rows(iid: int) -> int:
        async with get_db() as db:
            async with db.execute(
                "SELECT COUNT(*) FROM search_log WHERE instance_id=? AND action='info'",
                (iid,),
            ) as cur:
                row = await cur.fetchone()
        return int(row[0]) if row else 0

    before = asyncio.run(_count_info_rows(1))
    # Disable.  The newly-created instance starts enabled, so this is
    # the enabled -> disabled transition; no info row should be written.
    app.post("/settings/instances/1/toggle-enabled", headers=csrf_headers(app))
    after = asyncio.run(_count_info_rows(1))
    assert after == before
