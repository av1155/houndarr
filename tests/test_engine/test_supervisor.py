...
@pytest.mark.asyncio()
async def test_instance_loop_skip_grace_bypasses_initial_sleep(
    seeded_instances: None,
) -> None:
    """skip_grace=True fires the first cycle without waiting _STARTUP_GRACE_SECS.

    Path used by manual enable / edit-save / instance-create so the user
    sees dashboard updates in under a second instead of 10-40s.  The loop
    still sleeps between cycles (the second sleep in the test below, which
    we intercept to cancel cleanly), so ordinary pacing is untouched.
    """
    instance = _make_instance()
    sleep_calls: list[float] = []
    ...
    with (
        ...
        patch("houndarr.engine.supervisor.asyncio.sleep", side_effect=fake_sleep),
        patch(
            "houndarr.engine.supervisor.run_with_reconnect",
            new_callable=AsyncMock,
        ) as mock_reconnect,
    ):
        mock_reconnect.return_value = 60.0  # post-cycle sleep duration (ignored here)
        supervisor = Supervisor(master_key=MASTER_KEY)
        with pytest.raises(asyncio.CancelledError):  # noqa: PT012
            await supervisor._instance_loop(instance.core.id, skip_grace=True)  # noqa: SLF001

    # No grace sleep fired; the only sleep is the inter-cycle one.  The
    # inter-cycle value comes from run_with_reconnect's return (mocked to
    # 60.0), not from the grace constant.
    assert sleep_calls == [60.0]
    mock_reconnect.assert_called_once()


@pytest.mark.asyncio()
async def test_reconcile_instance_passes_skip_grace() -> None:
    """reconcile_instance always requests skip_grace=True on start.

    All reconcile call sites are user-initiated (toggle-enabled, edit-save,
    create) so the grace never applies.  Cold-boot uses Supervisor.start()
    which bypasses this method.
    """
    instance = _make_instance(enabled=True)

    with patch("houndarr.engine.supervisor.get_instance", return_value=instance):
        supervisor = Supervisor(master_key=MASTER_KEY)
    ...
    assert mock_start.call_args.kwargs["skip_grace"] is True


@pytest.mark.asyncio()
async def test_start_does_not_pass_skip_grace() -> None:
    """Cold-boot Supervisor.start() keeps the grace; only reconcile opts out.

    The grace exists specifically for the co-located *arr container boot
    race, which only applies to supervisor startup.  Accidentally
    propagating skip_grace=True through start() would defeat the point.
    """
    instance = _make_instance()

    with patch("houndarr.engine.supervisor.list_instances", return_value=[instance]):
        supervisor = Supervisor(master_key=MASTER_KEY)
        mock_start = AsyncMock(return_value=True)
        supervisor.start_instance_task = mock_start  # noqa: SLF001
        await supervisor.start()

    # start_instance_task is called once for the one enabled instance.  The
    # call must not opt out of the grace.
    assert mock_start.call_count == 1
    assert mock_start.call_args.kwargs.get("skip_grace", False) is False
